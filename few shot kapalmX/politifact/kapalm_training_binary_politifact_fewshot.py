#!/usr/bin/env python3
"""
Custom KAPALM (Knowledge grAPh Attentive Language Model) for Fake News Detection
Few-Shot Learning with PolitiFact Dataset
...
# Configuration for Binary Classification - PolitiFact Dataset with BERT
CONFIG = {
    ...
    'max_grad_norm': 1.0,
    'centrality_top_n': 100,  # Same as full-scale
    'hidden_dim': 768,
    'gat_heads': 8,
    'gat_dropout': 0.3,
    ...
    'base_seed': 42,
    'cache_dir': './cache_binary_politifact_fewshot',
    'save_path': './kapalm_binary_politifact_fewshot.pt',
    'log_file': './training_log_binary_politifact_fewshot.csv'
}
...
# Binary Label mapping for PolitiFact Dataset
LABEL_MAP = {
    0: 'real',
    1: 'fake'
}
...
class FakeNewsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text = str(row['FullText'])  # PolitiFact dataset uses 'FullText' column
        label = int(row['hoax'])     # PolitiFact dataset uses 'hoax' column
        ...
        # Load graph data - PolitiFact graphs are named: graph_politifact-XXXX.gpickle
...
class KAPALMModel(nn.Module):
    ...
    def __init__(self, config: Dict):
        ...
        if ADAPTERS_AVAILABLE:
            ...
            print(f"Using BERT ({config['model_name']}) with adapter tuning")
        else:
            # Use AutoModel for BERT compatibility
            ...
            print(f"Using BERT ({config['model_name']}) with last layer fine-tuning")
...
def create_few_shot_split(df_filtered, n_shot, seed=42):
    ...
    # Separate by class (using 'hoax' column for PolitiFact dataset)
    real_samples = df_filtered[df_filtered['hoax'] == 0]
    fake_samples = df_filtered[df_filtered['hoax'] == 1]
...
def main():
    ...
    """
    logger.info("=" * 100)
    logger.info("KAPALM Few-Shot Learning for PolitiFact Fake News Detection")
    logger.info("=" * 100)
    logger.info(f"Model: {CONFIG['model_name']} (BERT)")
    ...
    data_path = "../../dataset/politifact_01.csv"
    
    # Check if path exists, if not try alternative
    if not os.path.exists(data_path):
        data_path = "politifact_01.csv"
    
    df = pd.read_csv(data_path)
    logger.info(f"Loaded {len(df)} articles from dataset")
    
    # Filter data to only include samples with available graphs
    # Go up 2 levels from few_shot/politifact/ to reach processed_graphs/
    graphs_dir = Path("../../processed_graphs_politifact/successful")
    
    # Check if path exists
    if not graphs_dir.exists():
        # Try alternative paths
        alternative_paths = [
            Path("processed_graphs/successful"),
            Path("../processed_graphs/successful"),
        ]
    ...
    for graph_file in graphs_dir.glob("graph_*.gpickle"):
        # PolitiFact graphs are named like: graph_politifact-1234.gpickle
    ...
    class_counts = df_filtered['hoax'].value_counts()
    ...
    logger.info(f"\nLoading BERT tokenizer: {CONFIG['model_name']}")
    ...
    individual_df.to_csv('few_shot_individual_results_politifact.csv', index=False)
    logger.info(f"\nSaved all individual results to 'few_shot_individual_results_politifact.csv'")
    ...
    summary_df.to_csv('few_shot_summary_politifact.csv', index=False)
    ...
    with open('few_shot_results_politifact.json', 'w') as f:
        json.dump(json_results, f, indent=2)
    
    # Print final summary table
    logger.info(f"\n{'='*100}")
    logger.info("FINAL FEW-SHOT LEARNING SUMMARY (PolitiFact Dataset)")
    logger.info(f"{'='*100}")
    logger.info(f"Model: {CONFIG['model_name']} (BERT)")
    ...
    with open('few_shot_summary_politifact.txt', 'w') as f:
        f.write("KAPALM Few-Shot Learning Results - PolitiFact Dataset\n")
        f.write("="*100 + "\n\n")
        f.write("Experimental Setup:\n")
        f.write(f"  - Model: {CONFIG['model_name']} (BERT)\n")
    ...
    logger.info("\nAll results saved:")
    logger.info("  - few_shot_individual_results_politifact.csv (all 50 individual runs)")
    logger.info("  - few_shot_summary_politifact.csv (aggregated summary table)")
    logger.info("  - few_shot_results_politifact.json (detailed results with all scores)")
    logger.info("  - few_shot_summary_politifact.txt (human-readable summary)")
    logger.info("\nPolitiFact few-shot learning experiments completed!")
