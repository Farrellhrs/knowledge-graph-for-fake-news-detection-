...
Few-Shot Learning with GossipCop Dataset
...
# Configuration for Binary Classification - GossipCop Dataset with BERT
CONFIG = {
    ...
    'base_seed': 42,
    'cache_dir': './cache_binary_gossipcop_fewshot',
    'save_path': './kapalm_binary_gossipcop_fewshot.pt',
    'log_file': './training_log_binary_gossipcop_fewshot.csv'
}
...
class FakeNewsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text = str(row['FullText'])  # GossipCop dataset uses 'FullText' column
        label = int(row['hoax'])     # GossipCop dataset uses 'hoax' column
        ...
        # Load graph data - GossipCop graphs are named: graph_gossipcop-XXXXXXXXXX.gpickle
...
def create_few_shot_split(df_filtered, n_shot, seed=42):
    ...
    # Separate by class (using 'hoax' column for GossipCop dataset)
...
def main():
    ...
    """
    logger.info("=" * 100)
    logger.info("KAPALM Few-Shot Learning for GossipCop Fake News Detection")
    logger.info("=" * 100)
    ...
    if not graphs_dir.exists():
        ...
        alternative_paths = [
            Path("processed_graphs_gossipcop/successful"),
            Path("../processed_graphs_gossipcop/successful"),
        ]
    ...
    for graph_file in graphs_dir.glob("graph_*.gpickle"):
        # GossipCop graphs are named like: graph_gossipcop-1234567890.gpickle
    ...
    individual_df.to_csv('few_shot_individual_results_gossipcop.csv', index=False)
    logger.info(f"\nSaved all individual results to 'few_shot_individual_results_gossipcop.csv'")
    ...
    summary_df.to_csv('few_shot_summary_gossipcop.csv', index=False)
    ...
    with open('few_shot_results_gossipcop.json', 'w') as f:
        json.dump(json_results, f, indent=2)
    
    # Print final summary table
    logger.info(f"\n{'='*100}")
    logger.info("FINAL FEW-SHOT LEARNING SUMMARY (GossipCop Dataset)")
    logger.info(f"{'='*100}")
    logger.info(f"Model: {CONFIG['model_name']} (BERT)")
    ...
    with open('few_shot_summary_gossipcop.txt', 'w') as f:
        f.write("KAPALM Few-Shot Learning Results - GossipCop Dataset\n")
        f.write("="*100 + "\n\n")
    ...
    logger.info("\nAll results saved:")
    logger.info("  - few_shot_individual_results_gossipcop.csv (all 50 individual runs)")
    logger.info("  - few_shot_summary_gossipcop.csv (aggregated summary table)")
    logger.info("  - few_shot_results_gossipcop.json (detailed results with all scores)")
    logger.info("  - few_shot_summary_gossipcop.txt (human-readable summary)")
    logger.info("\nGossipCop few-shot learning experiments completed!")
