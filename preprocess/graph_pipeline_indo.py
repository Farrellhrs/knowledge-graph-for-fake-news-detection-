...
class GeminiEntityNormalizer:
    ...
    def __init__(self, model):
        self.model = model
        self.timeout_seconds = 60  # Timeout for Gemini API calls
    
    def _call_gemini_with_timeout(self, prompt, timeout=60):
        ...
        if thread.is_alive():
            ...
            return None, "Gemini API call timed out"
    ...
    def normalize_entities(self, entities_dict):
        ...
        try:
            print(f"  🤖 Sending normalization request to Gemini (timeout: {self.timeout_seconds}s)...")
            
            # Use timeout-wrapped call
            response, error = self._call_gemini_with_timeout(prompt, timeout=self.timeout_seconds)
            
            if error:
                print(f"  ⚠️ Gemini request failed: {error}")
                print("  ℹ️ Skipping normalization, proceeding with original entities")
                return entities_dict, {}
            
            if response is None:
                print("  ⚠️ No response from Gemini, skipping normalization")
                return entities_dict, {}
...
class GraphPipeline:
    ...
    def __init__(self, gemini_api_key, model_name="cahya/bert-base-indonesian-NER"):
        """
        ...
        # Determine device - use CPU if CUDA_VISIBLE_DEVICES is empty or if no GPU
        import os
        import torch
        if os.environ.get('CUDA_VISIBLE_DEVICES', None) == '':
            self.device = 'cpu'
            print("  ⚠️ Forcing CPU mode (CUDA_VISIBLE_DEVICES='')")
        ...
        self.ner_pipeline = pipeline(
            ...
            device=0 if self.device == 'cuda' else -1  # -1 for CPU, 0 for first GPU
        )
