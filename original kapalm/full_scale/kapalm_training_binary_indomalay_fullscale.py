...
Implements original KAPALM WITHOUT centrality-based pruning - uses full knowledge graphs
"""
...
# Configuration for Binary Classification - Indo-Malay Dataset (Original KAPALM - No Pruning)
CONFIG = {
    ...
    'warmup_steps': 500,
    'max_grad_norm': 1.0,
    # centrality_top_n removed - original KAPALM uses full graphs
    'hidden_dim': 768,
    'gat_heads': 8,
    'gat_dropout': 0.3,
    ...
    'seed': 42,
    'cache_dir': './cache_binary_indomalay',
    'save_path': './kapalm_binary_indomalay.pt',
    'log_file': './training_log_binary_indomalay.csv'
}
...
class GraphProcessor:
    """Handles graph loading and conversion to PyTorch Geometric format (Original KAPALM - No Pruning)"""
    ...
    def compute_centrality_pruning(self, G: nx.Graph) -> nx.Graph:
        """Original KAPALM: No centrality pruning, return full graph"""
        return G.copy()
...
def train_model(model, train_loader, val_loader, config):
    ...
    for epoch in range(config['num_epochs']):
        ...
        logger.info(f'  Epoch Time: {epoch_time_sec:.2f} sec')
    ...
    logger.info(f'Total training time: {training_time_sec:.2f} sec ({training_time_sec / 60:.2f} min)')
    
    return model, training_log, training_time_sec
...
def main():
    ...
    logger.info("Starting KAPALM training for binary fake news detection - Indo-Malay Dataset")
    logger.info(f"Using IndoBERT model: {CONFIG['model_name']}")
    logger.info("Binary Classification: 0=Real vs 1=Fake (Hoax)")
    logger.info(f"CUDA available: {torch.cuda.is_available()}")
    
    # Load data
    logger.info("Loading Indo-Malay dataset...")
    data_path = "dataset_sunda_malay__filtered_46k_no_dup.csv"
    df = pd.read_csv(data_path)
    
    # Filter data to only include samples with available graphs
    # Go up 2 levels from full_scale/indo_malay/ to reach processed_graphs_indo_malay/
    ...
    for graph_file in graphs_dir.glob("graph_*.gpickle"):
        # Extract the numeric ID (e.g., "1" from "graph_1.gpickle")
    ...
    # Data split - using 'label' column for Indo-Malay dataset
    ...
    # Initialize components - use AutoTokenizer for IndoBERT
    tokenizer = AutoTokenizer.from_pretrained(CONFIG['model_name'])
    graph_processor = GraphProcessor(CONFIG['cache_dir'])
    ...
    model, training_log, training_time_sec = train_model(model, train_loader, val_loader, CONFIG)
    ...
    with open('final_results_binary_indomalay.json', 'w') as f:
        json.dump({k: v for k, v in results.items() if k != 'classification_report'}, f, indent=2)
    
    with open('classification_report_binary_indomalay.txt', 'w') as f:
        f.write(str(report))
    
    logger.info(f"Training time: {training_time_sec:.2f} sec ({training_time_sec / 60:.2f} min)")
    logger.info(f"Total runtime: {total_runtime_sec:.2f} sec ({total_runtime_sec / 60:.2f} min)")
    logger.info("Indo-Malay binary classification training completed! Results saved.")
