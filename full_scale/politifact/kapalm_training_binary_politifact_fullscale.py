...
CONFIG = {
    ...
    'gat_dropout': 0.3,
    'classifier_dropout': 0.3,  # Reduced dropout for binary classification
    'num_labels': 2,  # Binary classification: 0 (True/Mostly True) vs 1 (False/Mostly False)
    'seed': 42,
    'cache_dir': './cache_binary',
    'save_path': './kapalm_binary.pt',
    'log_file': './training_log_binary.csv'
}
...
def main():
    ...
    logger.info("Starting KAPALM training for binary fake news detection - PolitiFact Dataset")
    logger.info("Binary Classification: 0=Real vs 1=Fake")
    logger.info(f"CUDA available: {torch.cuda.is_available()}")
    
    # Load data
    logger.info("Loading PolitiFact dataset...")
    ...
    with open('final_results_binary_politifact.json', 'w') as f:
        json.dump({k: v for k, v in results.items() if k != 'classification_report'}, f, indent=2)
    
    with open('classification_report_binary_politifact.txt', 'w') as f:
        f.write(str(report))
    
    logger.info("PolitiFact binary classification training completed! Results saved.")
