...
from dataset import GossipCopDataset, load_and_filter_dataset
...
def run_ablation_experiment(ablation_mode: AblationMode) -> dict:
    ...
    df = load_and_filter_dataset(PATHS['dataset'], PATHS['graphs_dir'])
...
def print_final_report(results: dict):
    ...
    print("Dataset: GossipCop Fake News Detection")
    print("=" * 100)
    ...
    # Analysis
    print("\n" + "=" * 100)
    print("📝 ANALYSIS: IMPLICATIONS FOR GOSSIPCOP KNOWLEDGE UTILIZATION")
    print("=" * 100)
    print("""
1. FULL MODEL vs w/o GRAPH POOLING (w/o GP):
   - If Full > w/o GP: Fine-grained knowledge (attentive pooling) helps capture 
    entity-specific relationships important for GossipCop fake news detection.
...
3. FINE-GRAINED ONLY vs COARSE ONLY:
   - If Fine > Coarse: Local entity neighborhoods are more informative for 
         GossipCop fake news, suggesting entity-centric misinformation patterns.
...
4. BOTH REPRESENTATIONS vs SINGLE:
   - If Full > max(Fine, Coarse): Complementary information from both scales
         is needed for optimal detection in GossipCop context.
     - This would suggest fake news in GossipCop both distorts local entity
...
5. IMPACT OF GRAPH PRUNING:
   - Compare Coarse Only (no pruning) vs Full Model (with pruning):
    - If pruning helps: Noise reduction benefits GossipCop knowledge graphs.
...
def main():
    parser = argparse.ArgumentParser(description='KAPALM Ablation Study for GossipCop Dataset')
    ...
    logger.info("=" * 80)
    logger.info("KAPALM ABLATION STUDY - GOSSIPCOP FAKE NEWS DETECTION")
