# KAPALM Ablation Study - GossipCop Fake News Detection

## Overview

This ablation study analyzes the contribution of different components in the KAPALM model for GossipCop fake news detection.
...
- **If Full > w/o GP**: Fine-grained knowledge via attentive pooling captures entity-specific relationships critical for GossipCop fake news detection. This suggests misinformation often targets specific entities or makes false claims about particular relationships.
...
- **If Full > w/o IN**: The learnable interaction node provides valuable aggregated graph context beyond simple mean pooling. It captures complex interactions between knowledge graph elements that are important for GossipCop fake news patterns.
...
- **If Fine > Coarse**: Local entity neighborhoods (pruned subgraphs around key entities) are more informative. GossipCop fake news likely involves entity-centric misinformation patterns.
...
- **If Full > max(Fine, Coarse)**: Both scales of knowledge are complementary and necessary. GossipCop fake news both distorts local entity facts and disrupts broader knowledge patterns.
...
- **Coarse Only (original graph) vs Full (with centrality pruning)**:
  - If pruning helps: GossipCop knowledge graphs contain noise that pruning removes.
  - If pruning hurts: Full graph structure contains important peripheral information.

### GossipCop Specific Considerations

For GossipCop fake news detection specifically:
- Entertainment and celebrity claims may be highly entity-centric
- Knowledge graphs can capture relationships among people, events, and organizations
