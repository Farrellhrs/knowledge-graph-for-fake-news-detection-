...
class PolitiFactDataset(Dataset):
    """
    Dataset class for PolitiFact fake news detection with graph data.
...
def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize PolitiFact-like datasets to id/text/label schema."""
    id_candidates = ['id', 'ID', 'news_id', 'article_id']
    text_candidates = ['FullText', 'fulltext', 'text', 'content']
    label_candidates = ['hoax', 'label', 'is_fake', 'target']
...
def load_and_filter_dataset(dataset_path: str, graphs_dir: str):
    ...
    print(f"Class distribution: True={len(df_filtered[df_filtered['label']==0])}, "
          f"Fake={len(df_filtered[df_filtered['label']==1])}")
    
    return df_filtered


# Backward-compatible aliases (copied code may still import older names)
GossipCopDataset = PolitiFactDataset
IndoMalayDataset = PolitiFactDataset
