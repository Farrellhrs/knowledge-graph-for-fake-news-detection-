# KAPALM Ablation Study - PolitiFact Fake News Detection

## Overview

This ablation study analyzes the contribution of different components in the KAPALM model for PolitiFact fake news detection.
...
- **If Full > w/o GP**: Fine-grained knowledge via attentive pooling captures entity-specific relationships critical for PolitiFact fake news detection. This suggests misinformation often targets specific entities or makes false claims about particular relationships.
- **If Full ≈ w/o GP**: The fine-grained details are redundant; the coarse-grained global structure is sufficient for this dataset.

#### 2. Full Model vs w/o Interaction Node (w/o IN)
- **If Full > w/o IN**: The learnable interaction node provides valuable aggregated graph context beyond simple mean pooling. It captures complex interactions between knowledge graph elements that are important for PolitiFact fake news patterns.
...
- **If Fine > Coarse**: Local entity neighborhoods (pruned subgraphs around key entities) are more informative. PolitiFact fake news likely involves entity-centric misinformation patterns.
...
- **If Full > max(Fine, Coarse)**: Both scales of knowledge are complementary and necessary. PolitiFact fake news both distorts local entity facts and disrupts broader knowledge patterns.
...
- **Coarse Only (original graph) vs Full (with centrality pruning)**:
  - If pruning helps: PolitiFact knowledge graphs contain noise that pruning removes.
  - If pruning hurts: Full graph structure contains important peripheral information.

### PolitiFact Specific Considerations

For PolitiFact fake news detection specifically:
- Political claims often rely on entities such as public figures, institutions, and events
- Knowledge graphs can capture factual relationships among these entities
- The dual-scale approach may help capture both:
  - **Coarse**: Global topic/claim coherence
  - **Fine**: Specific entity-relationship inconsistencies
