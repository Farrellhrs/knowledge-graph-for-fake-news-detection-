...
Indo-Malay Fake News Detection
...
def print_final_report(results: dict):
    ...
    print("Dataset: Indo-Malay Fake News Detection")
    print("=" * 100)
    ...
    # Analysis
    print("\n" + "=" * 100)
    print("📝 ANALYSIS: IMPLICATIONS FOR INDO-MALAY KNOWLEDGE UTILIZATION")
    print("=" * 100)
    print("""
1. FULL MODEL vs w/o GRAPH POOLING (w/o GP):
   - If Full > w/o GP: Fine-grained knowledge (attentive pooling) helps capture 
     entity-specific relationships important for Indo-Malay fake news detection.
...
3. FINE-GRAINED ONLY vs COARSE ONLY:
   - If Fine > Coarse: Local entity neighborhoods are more informative for 
     Indo-Malay fake news, suggesting entity-centric misinformation patterns.
...
4. BOTH REPRESENTATIONS vs SINGLE:
   - If Full > max(Fine, Coarse): Complementary information from both scales
     is needed for optimal detection in Indo-Malay context.
   - This would suggest fake news in Indo-Malay both distorts local entity
...
5. IMPACT OF GRAPH PRUNING:
   ...
   - If pruning helps: Noise reduction benefits Indo-Malay knowledge graphs.
...
def main():
    parser = argparse.ArgumentParser(description='KAPALM Ablation Study for Indo-Malay Dataset')
    ...
    logger.info("=" * 80)
    logger.info("KAPALM ABLATION STUDY - INDO-MALAY FAKE NEWS DETECTION")
